...
def _argnums_partial(dyn_argnums, fixed_args, *dyn_args, **kwargs):
  ...
  for i, arg in zip(dyn_argnums, dyn_args):
    args[i] = arg
  fixed_args = iter(fixed_args)
  args = [next(fixed_args).val if x is None else x for x in args]
...
def argnames_partial(f, dyn_argnames, kwargs):
  ...
  fixed_kwargs = tuple((k, wrap_hashably(v)) for k, v in kwargs.items()
...
def _argnames_partial(fixed_kwargs: WrapKwArgs, *args, **dyn_kwargs):
  kwargs = dict(fixed_kwargs.val.items(), **dyn_kwargs)
  ans = yield args, kwargs
  yield ans
